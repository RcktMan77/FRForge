# 2D smooth order, discontinuous, curved, freestream, capturing suites.
...
function run_p32_curved_suite()
    ...
    c2 = run_freestream_preservation_2d(;
        p = 3,
        nx = 4,
        ny = 4,
        curved = true,
    ...
    push!(cases, c2)
    !c2["pass"] &&
        push!(hard_fails, "curved freestream failed res=$(c2["metrics"]["residual_max"])")
...
function run_p31_2d_capturing_suite()
    ...
    c_null, _, _ =
        run_euler2d_discontinuous(; p = 1, nx = 12, ny = 12, t_final = 0.05, method_name = "null")
    push!(cases, c_null)
